...
h = ecog_prf_plotPRFelations(prfs,prfs_corr,relt,flds,selroi,rois,opts);
hF = [hF h];
...
h = ecog_prf_plotPRFelations(prfs,prfs_corrx,relt,flds,selroi,rois,opts);
hF = [hF h];
...
opts.showfitted  = strtok(fitting,'-');
opts.fit_alpha   = alpha;
h = ecog_prf_plotPRFelations(prfsC,prfsC_corr,relt,flds,roisC,roisC,opts);
hF = [hF h];
