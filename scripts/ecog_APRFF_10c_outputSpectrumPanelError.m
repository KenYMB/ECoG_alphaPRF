...
nexttile;
if corrslope
    bb_comp_lin = 10.^func_model([-bb_amp,0,log10(iaf),iaw],log10(f)-log10(iaf),slope);
    bb_comp_lin(f<=0) = bb_comp_lin(find(f>0,1));   % exclude invalid f
else
    bb_comp_lin = 10.^bb_amp;
end
h=errorshade([f,f],[data_base,data_fit./bb_comp_lin],...
      [se_l_base,se_l_fit./bb_comp_lin],[se_u_base,se_u_fit./bb_comp_lin],...
      ["k","#A06440"],'LineWidth',4);
h(2,:) = [];
set(gca,'YScale','log');
hold on;
...
ylim(y_lin);
if plotiaf, plot(f_detail([iaidx iaidx]),y_lin,'k:','LineWidth',2); end
if plotarrow, plot_arrow(f_detail(iaidx),aa_base(iaidx),f_detail(iaidx), aa_fit(iaidx)./10.^bb_amp,'LineWidth',1.6,'absheadheight',abs(diff([aa_base(iaidx), aa_fit(iaidx)./10.^bb_amp]))*0.2,'absheadwidth',1.0); end
% xticks(unique([min(xlim) xticks]));
ylim(y_lin);
...
figureName = sprintf('%s-error',figureName);
