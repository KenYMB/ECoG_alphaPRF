...
if 0%exist([filepath '.mat'],'file')
 ...
 nroi = length(rois);
     ...
     subjects        = prf_all_a.subjects;
     nsbj = length(subjects);
 mn_keep_roi = cell(nsbj,nroi); mn_discard_roi = cell(nsbj,nroi);
 n_keep_roi  = cell(nsbj,nroi); n_discard_roi  = cell(nsbj,nroi);
 for isbj=1:nsbj
 for iboot=1:nboot       % ~3.4m for 2000 iteration
     %%-- assign roi label based on probability
     [~,prf_all_a]   = ecog_rearrangePRF(prf_params_a(isbj),va_area,'prob');
     ...
     elec_ok   = prf_params_bb{isbj}.xval(boot_keep,:)>threshold;
     for iroi=1:nroi
         ...
         if any(elec_keep)
             mn_keep_roi{isbj,iroi}(iboot) = mean(xval(elec_keep));
         else
             mn_keep_roi{isbj,iroi}(iboot) = nan;
         end
         n_keep_roi{isbj,iroi}(iboot)  = sum(elec_keep);         % electrode numbers (save even for invalid condition)
         if any(elec_discard)
             mn_discard_roi{isbj,iroi}(iboot) = mean(xval(elec_discard));
         else
             mn_discard_roi{isbj,iroi}(iboot) = nan;
         end
         n_discard_roi{isbj,iroi}(iboot)  = sum(elec_discard);   % electrode numbers (save even for invalid condition)
     end
 end
 end
 
 saveauto(filepath,'rois','mn_keep_roi','mn_discard_roi','n_keep_roi','n_discard_roi','nboot','threshold','subjects');
end
...
nboot     = 5000;
%-- compute xR2_a in ROIs (wang_prob)
filename = sprintf('all_chansel-wangprob%s-%s%s_thresh%02d-indrev',R2mode,selectchs,postfix,threshold);
filepath = fullfile(SetDefaultAnalysisPath('DAT',prfstatPth),filename);
...
if 0%exist([filepath '.mat'],'file')
 ...
 nroi = length(rois);
     %-- get subject list
     [~,prf_all_a]   = ecog_rearrangePRF(prf_params_bb,va_area);
     subjects        = prf_all_a.subjects;
     nsbj = length(subjects);
 mn_keep_roi = cell(nsbj,nroi); mn_discard_roi = cell(nsbj,nroi);
 n_keep_roi  = cell(nsbj,nroi); n_discard_roi  = cell(nsbj,nroi);
 ...
 for iboot=1:nboot       % ~3.4m for 2000 iteration
     %%-- assign roi label based on probability
     [~,prf_all_bb]   = ecog_rearrangePRF(prf_params_bb(isbj),va_area,'prob');
     %%-- randomly select channels
     ...
     elec_ok   = prf_params_a{isbj}.xval(boot_keep,:)>threshold;
     for iroi=1:nroi
         switch lower(rois{iroi})
             case {'low','v1-v3'}
                 catrois = {'V1';'V2';'V3'};
