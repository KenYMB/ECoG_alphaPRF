...
clear alphaType broadbandType
    ...
    SetDefault('noalphathresh',false);
...
if isnan(threshold_a),  eclimit_a = nan;
else,                   eclimit_a = eclimit;
end
...
if exist([filepath '.mat'],'file')
 ...
 for iboot=1:nboot       % 4m for 2,000 iteration
     ...
     elec_ok = ~(prf_all_bb.xval(boot_keep)<=threshold_bb | prf_all_a.xval(boot_keep)<=threshold_a ...
         | prf_all_bb.ecc(boot_keep) >= eclimit | prf_all_a.ecc(boot_keep) >= eclimit_a);  % use tilde for nan
     %-- computation in roi
     for iroi = 1:nroi
         %-- roi selection
