...
avgdat    = cell(size(coh_bb0));
avgbsl    = cell(size(coh_bb0));
...
subfld = fieldnames(prf_params_bb{isbj});
...
%% parameter for plot
   ...
   flng  = length(iconn.f);
   chlng = height(prf_all_bb.channels);
...
cohbb = zeros(size(avgdat,[3 1 2]));
bslbb = zeros(size(avgbsl,[3 1 2]));
for ich = 1:chlng
    cohbb(:,ich,:)  = permute(avgdat(ich,:,:,1),[3 1 2 4]);
    bslbb(:,ich,:)  = permute(avgbsl(ich,:,:,1),[3 1 2 4]);
end
cohbb  = decimate_col(cohbb,decN,[],1);
...
% end
% 
% chcavgtsBBdat = mean(chcavgtsBBdat,'all','omitnan');
% chcavgtsBBbsl = mean(chcavgtsBBbsl,'all','omitnan');
...
cohdatBBbsl = mat2cell(cohbb,size(cohbb,1),ones(size(cohbb,2),1),size(cohbb,3));
cohdatBBprf = cohdatBBbsl;
cohdatBBout = cohdatBBbsl;
for ich=1:chlng
    cohdatBBbsl{ich} = cohdatBBbsl{ich}(blankidx,:);
    cohdatBBprf{ich} = cohdatBBprf{ich}(prfidx(:,ich),:);
    cohdatBBout{ich} = cohdatBBout{ich}(~blankidx&~prfidx(:,ich),:);
end

%%% Take average in each elec
avgtsBBbsl = cellfun(@mean,cohdatBBbsl(selch),repmat({1},1,sum(selch)),'UniformOutput',false)';
avgtsBBprf = cellfun(@mean,cohdatBBprf(selch),repmat({1},1,sum(selch)),'UniformOutput',false)';
avgtsBBout = cellfun(@mean,cohdatBBout(selch),repmat({1},1,sum(selch)),'UniformOutput',false)';
...
%     y1 = median(ys,1,'omitnan');
  ...
  uistack(hl(2),'top');
%     hb = plot(xlim,[1,1].*(chcavgtsBBdat-chcavgtsBBbsl),'k--','LineWidth',1.2);
%     uistack(hb,'bottom');
...
%     y1 = median(ys,1,'omitnan');
  ...
  uistack(hl(3),'top');
%     hb = plot(xlim,[1,1].*chcavgtsBBdat,'k--','LineWidth',1.2);
%     uistack(hb,'bottom');
set(gca,'FontSize',FntSiz);
...
%% anova
valch  = ~any(isnan([avgtsBBbsl avgtsBBprf avgtsBBout]),2);
avgtsBB = [avgtsBBbsl(valch,:);avgtsBBprf(valch,:);avgtsBBout(valch,:)];
ps = anova2(avgtsBB,sum(valch));
...
dcohbb = cohbb(:,:,irnd) - bslbb(:,:,irnd);

figure('Menubar','none');
yyaxis left
scatter(dcohbb(:,selch),dattsbb(:,selch),'LineWidth',1);
hold on
yy1 = ylim;
 if hglblank,  scatter(dcohbb(blankidx,selch),dattsbb(blankidx,selch),[],mkblcl(plcol(1,:)),'LineWidth',1);  end
 if hglinprf,  scatter(dcohbb(prfidxbb&selch),dattsbb(prfidxbb&selch),[],plcol(1,:),'LineWidth',2.0);  end
yyaxis right
scatter(dcohbb(:,selch),dattsa(:,selch),'LineWidth',1);
hold on
yy2 = ylim;
 if hglblank,  scatter(dcohbb(blankidx,selch),dattsa(blankidx,selch),[],mkblcl(plcol(2,:)),'LineWidth',1);  end
 if hglinprf,  scatter(dcohbb(prfidxa&selch),dattsa(prfidxa&selch),[],plcol(2,:),'LineWidth',2.0);  end
hA = get(gca,'YAxis');
...
if showelps
  yyaxis left
  h=error_ellipse(cov(reshape(dcohbb(:,selch),[],1),reshape(dattsbb(:,selch),[],1)),...
                [median(reshape(dcohbb(:,selch),[],1)),median(reshape(dattsbb(:,selch),[],1))],'conf',1-alpha);
  h.LineWidth = 2.6;  h.LineStyle = '-.'; h.Marker='none';
  h.Color = [0 0 1];
  yyaxis right
  h=error_ellipse(cov(reshape(dcohbb(:,selch),[],1),reshape(dattsa(:,selch),[],1)),...
                [median(reshape(dcohbb(:,selch),[],1)),median(reshape(dattsa(:,selch),[],1))],'conf',1-alpha);
  h.LineWidth = 2.6;  h.LineStyle = '-.'; h.Marker='none';
  h.Color = [1 0 0];
end
...
figure('Menubar','none');
h1=scatter(dcohbb(~prfidx&~blankidx&selch),datts(~prfidx&~blankidx&selch),[],plcol(5,:),'LineWidth',1.0);
hold on
h2=scatter(dcohbb(prfidx&selch),datts(prfidx&selch),[],plcol(7,:),'LineWidth',1.0);
yy1=ylim;
...
%-- show ellipse
if showelps
  h=error_ellipse(cov(reshape(dcohbb(~prfidx&~blankidx&selch),[],1),reshape(datts(~prfidx&~blankidx&selch),[],1)),...
                [median(reshape(dcohbb(~prfidx&~blankidx&selch),[],1)),median(reshape(datts(~prfidx&~blankidx&selch),[],1))],'conf',1-alpha);
  h.LineWidth = 2.6;  h.LineStyle = '--'; h.Marker='none';
  h.Color = [0 1 0];
  
  h=error_ellipse(cov(reshape(dcohbb(prfidx&selch),[],1),reshape(datts(prfidx&selch),[],1)),...
                [median(reshape(dcohbb(prfidx&selch),[],1)),median(reshape(datts(prfidx&selch),[],1))],'conf',1-alpha);
  h.LineWidth = 2.6;  h.LineStyle = '-.'; h.Marker='none';
  h.Color = [1 0 0];
...
A=ylabel('Broadband Power Ratio');
xlabel(sprintf('Difference of Coherence from BLANK')); %  - Peak Alpha Frequency
% title(sprintf('%s',useChans));
...
cohdatbsl = mat2cell(cohbb(:,:,irnd),size(cohbb,1),ones(size(cohbb,2),1));
cohdatprf = cohdatbsl;
cohdatout = cohdatbsl;
for ich=1:chlng
...
ylabel(sprintf('Coherence - Average in Broadband'));
