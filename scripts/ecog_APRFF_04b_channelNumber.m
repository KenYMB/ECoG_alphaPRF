...
wang_early   = startsWith(fieldnames(summary(channels)),'wangprob_') & endsWith(fieldnames(summary(channels)),{'V1','V2','V3'});
wang_dorsal  = startsWith(fieldnames(summary(channels)),'wangprob_') & endsWith(fieldnames(summary(channels)),{'V3a','V3b','LO1','LO2','TO','IPS'});
wang_ventral = startsWith(fieldnames(summary(channels)),'wangprob_') & endsWith(fieldnames(summary(channels)),{'hV4','VO','PHC'});

%-- Subject
fprintf('%d electrodes from %d subjects:%s\n',sum(elec_ok),numel(unique(channels.subject_name(elec_ok))),...
                                              sprintf(' %s',unique(channels.subject_name(elec_ok))));
for subject = unique(channels.subject_name(elec_ok))'
    fprintf('\t%d electrodes from %s\n',sum(elec_ok&ismember(channels.subject_name,subject)),subject);
end
%-- ROI
Label = 'V1-V3'; chanidx = elec_ok&sum(channels{:,wang_early},2)>0;
fprintf('%d electrodes in %s from%s\n',sum(chanidx),Label,sprintf(' %s',unique(channels.subject_name(chanidx))));
...
for subject = string(subjectList')
    ...
    fprintf('%s\t# Vis:%3d\t# Sel:%3d\tArea:%s\n',subject,sum(elec_sbj),sum(elec_sbj&elec_ok),...
        sprintf(' %s',unique(channels.wangarea(elec_sbj&elec_ok))));
