...
if issaveplot
gaussianModes = {'dog','gs','og'};
modelNames = ["Difference of Gaussians (FULL)","Difference of Gaussians","One Gaussian"];
else
gaussianModes = {'gs','og'};
modelNames = ["Difference of Gaussians","One Gaussian"];
end
...
prf_all    = struct();
threshold  = struct();
for ii=length(gaussianModes):-1:1
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% GS vs OG, GS vs DoG, DoG vs OG
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
modelsetsList = nchoosek(1:numel(modelNames),2);
for nn = 1:size(modelsetsList,1)
modelsets = modelsetsList(nn,:);

modelVS = [prfmodel '-' strjoin(gaussianModes(modelsets),'VS')];
ecog_APRFF_INITc_postfix;
...
if ispltall
    plcol_gray = [plcol_wang .* 1  +  ones(1,3).*0.7 .* 2]./3;
end
