...
if issaveplot
    plotsavedir    = fullfile(figPth, 'pRF-representative');
...
hF = horzcat(hF,ecog_plotGridPRFts(mdlall.datats,mdlall.stimulus,prfall,el(ii), opt));
...
%         if ismember(prfall.channels.name(el(ii)),'GB103') && ismember(prfall.channels.subject_name(el(ii)),'som726')
%             ylim(log10([.2 2]));
%         end
end
