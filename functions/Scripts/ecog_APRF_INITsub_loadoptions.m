...
if ~AutoFILLalphaType
    UPDATEcomputeOpt = UPDATEcomputeOpt | UPDATEcomputeOptFnc('allowmixbeta', iscell(alphaType) && length(alphaType)>1 && ~all(matches(alphaType,alphaType{1})));
...
%% subfunction
function    isupdated = UPDATEcomputeOptFnc(VARNAME,VALUE)
prevval   = [];
