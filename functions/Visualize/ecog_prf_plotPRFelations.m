function   varargout = ecog_prf_plotPRFelations(prfs,prfs_corr,reltarget,varargin)
% h = ecog_prf_plotPRFelations(prfs,prfs_corr,Target,[parms,conds,selrois,roiList,opts])
...
%   ecog_prf_plotPRFelations(prfs,prfs_corr,Target,parms,conds,selrois,roiList,opts)
% 
% Example 2:
%   Target  = {'ecc_rfsize'};
%   parms   = {'ang','ecc','rfsize'};
...
if noclmn
    ix = ceil(sqrt(length(selroi)));    iy = floor(sqrt(length(selroi)));
else
    ix = length(selroi);  iy = length(lpprops);
end

if noclmn, figSiz = [ix*450+40 iy*330+40];
else,      figSiz = [ix*300+100 iy*300+100];
end
...
for iroi = selroi
    ...
    for sbprop = lpprops
        ...
        if isfield(prfs_corr,sbfldF)
            ...
            if showfitted==3     % error if showfitted = 'fitlm'
                close(hF(end));
                error('prfs_corr must have %s field',sbfldF);
            end
            datF = nan;
        end
...
% if rel4conds && ~isoverplt
    ...
    if noclmn
    ylabel(hT,yglabel,'HorizontalAlignment','center','FontSize',FontSize);
    else
    axPos = hT.Children(1).Position;
    txpos = [1.01,(0.5-axPos(2))/axPos(4)];
    text(gca,txpos(1),txpos(2),yglabel,'Unit','normalized','HorizontalAlignment','center','VerticalAlignment','bottom','Rotation',-90,'FontSize',FontSize);
    end
% end
%     
