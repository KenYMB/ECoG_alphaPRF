...
for ii = 1 : numel(data)
    ...
    if compute
        ...
        if isempty(opts.channels)
            opts.channels = true(height(channels),1);
        elseif iscellstr(opts.channels) || ischar(opts.channels)
            opts.channels = cellstrfind(channels.name,opts.channels);
        end
        epochs   = epochs(opts.channels,:,:);
        channels = channels(opts.channels,:);
        ...
        %%-- stimuli epochs
        [f,crossspectra] = ...
            ecog_crossspectra(epochs(:,trialIndex,:),[],fft_w,fft_t,fft_ov,srate,reg_erp,target_freq,outtype);
        %%-- exclude frequencies
        f_rej  = false(size(f));
...
if strcmpi(targetBAND,'other')
    postfix = sprintf('%s-%g-%gHz',postfix,target_freq);
else
