...
%                     'headwidth':  relative to complete arrow length, default value is 0.07
%                     'headheight': relative to complete arrow length, default value is 0.15
% 
%                     alternative options are:
%                     'absheadwidth':  absolute arrow head width
%                     'absheadheight': absolute arrow head height
%
...
% 20210119 Yuasa: bug fix, modify bounds, & add alternative options
...
max_length  = inf;    % ex) 22 pixel
max_width   = inf;    % ex) 10 pixel
    ...
    for c = 1:floor(length(varargin)/2)
        try
            switch lower(varargin{c*2-1})
             % head properties
            case 'headheight',alpha = max( min( varargin{c*2},1 ),0 );
            case 'headwidth', beta  = max( varargin{c*2},0 );
            end
        catch
...
% check if absolute value is set
if ~isempty( varargin )
    for c = 1:floor(length(varargin)/2)
        try
            switch lower(varargin{c*2-1})
             % head properties
            case 'absheadheight',head_length = min(varargin{c*2},line_length);
            case 'absheadwidth', head_width  = varargin{c*2};
            end
        catch
            fprintf( 'unrecognized property or value for: %s\n',varargin{c*2-1} );
        end
