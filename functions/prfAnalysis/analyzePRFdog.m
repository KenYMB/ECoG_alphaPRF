...
%       'LFS' uses Large-Field Suppression model (One Gaussian with Global Suppression) 
%     default: 'DoG'.
...
%       2 means use generic sLFS seed (slightly Large-Field Suppression for DoG optimization)
%       3 means use generic LFS seed or best LFS seed based on super-grid for seedmode=2 (Large-Field Suppression)
%     default: [0 1 2] for DoG model, [1] for OG model, [3] for LFS model. 
...
% 2014/10/21 - update model name GS to LFS
...
% Dependency: <analyzePRF>, setboundsinfunc, replelement
...
if ~isfield(options,'gaussianmode') || isempty(options.gaussianmode)
  ...
  assert(ismember(lower(options.gaussianmode),{'dog','og','lfs','gs'}),'gaussianmode ''%s'' is invalid',options.gaussianmode);
